...
class AirHockey(object):
    def __init__(self, display=0, dim=Dimensions()):
        ...
        self.top_ai          = MachineLearningAI(self.top_mallet, self.puck, mode='top', dim=self.dim)
#        self.bottom_ai       = MachineLearningAI(self.bottom_mallet, self.puck, mode='bottom', dim=self.dim)
...
#        self.forces.add(self.bottom_mallet, PlayerForce(player=0))
    ...
    def step(self, x, y, delta_time=1, actions=None, n_steps=3):
...
#            self.bottom_ai.move()
            ...
            self.bottom_ai_force.set_force(np.array((x, y), dtype=np.float32))
            ...
            self.points = {self.puck: [], self.top_mallet: [], self.bottom_mallet: []}
            for body in self.bodies:
                for border in body.borders:
                    self.points[body].append(Collision.circle_line(body, border))
    ...
    def _draw(self, puck, top_mallet, bottom_mallet, debug=False):        
        ...
        self.screen.blit(self.top_arm_sprite, 
                    top_mallet - np.array((self.dim.mallet_radius, self.top_arm_sprite.get_size()[1] - self.dim.mallet_radius), dtype=np.float32))
#        screen.blit(self.bottom_arm_sprite, bottom_mallet - self.dim.mallet_radius)

        pygame.draw.line(self.screen, (0, 0, 16), [self.dim.rink_left, bottom_mallet[1]], [self.dim.rink_right, bottom_mallet[1]], 24) 
        pygame.draw.line(self.screen, (0, 0, 16), [(self.dim.table_left+self.dim.rink_left)/2, self.dim.table_bottom], [(self.dim.table_left+self.dim.rink_left)/2, self.dim.center[1]], 24) 
        pygame.draw.line(self.screen, (0, 0, 16), [(self.dim.table_right+self.dim.rink_right)/2, self.dim.table_bottom], [(self.dim.table_right+self.dim.rink_right)/2, self.dim.center[1]], 24) 
    ...
    def _draw_collision_points(self):
        for obj, points in self.points.items():
            radius = 6
            if obj == self.puck:
                color = C.puck
                radius = 10
            elif obj == self.top_mallet:
                color = C.top_mallet
            elif obj == self.bottom_mallet:
    ...
    def _render(self, debug=0):        
        ...
        if debug: self._draw_collision_points()
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        observations = env.step(x, y)
        x, y = model.predict(observations)
        x = np.argmax(x) - 1
        y = np.argmax(y) - 1
        print(x, y)
    pygame.quit ()